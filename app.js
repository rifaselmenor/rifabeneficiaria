...
let PRECIO_BOLETO      = 5;      
let MINIMO_BOLETOS     = 2; // Bajé el mínimo por defecto a 2, el admin puede cambiarlo
const TOTAL_BOLETOS      = 100;  
const BOLETOS_POR_PAGINA = 100; // Todo en una sola página
...
function configurarBotonesDinamicos() {
  ...
  if (btnRestar && btnSumar && displayDiv) {
    ...
    btnRestar.onclick = () => {
      if (cantidadAzar > MINIMO_BOLETOS) { cantidadAzar -= 1; displayDiv.textContent = cantidadAzar; } 
      else { showToast(`El mínimo es ${MINIMO_BOLETOS}`); }
    };
    btnSumar.onclick = () => { cantidadAzar += 1; displayDiv.textContent = cantidadAzar; };
  }
}

async function loadTickets() {
  for (let i = 0; i < TOTAL_BOLETOS; i++) {
    let numStr = i.toString().padStart(2, '0'); // Cambiado a 2 dígitos
    ticketStates.set(numStr, 'available');
  }
  try {
    const { data, error } = await db.from('tickets').select('numero,estado').in('estado', ['pendiente','vendido']).range(0, 100);
...
function toggleTicket(numStr) {
  if (selectedTickets.has(numStr)) selectedTickets.delete(numStr);
  else selectedTickets.add(numStr);
  renderGrid(); updateUI();
}
...
function randomSelect() {
  ...
  for (let i = 0; i < cantidadAzar; i++) selectedTickets.add(pool[i]);
...
function updateUI() {
  ...
  if(btnPagar) {
      if (count >= MINIMO_BOLETOS) { btnPagar.disabled = false; btnPagar.style.opacity = '1'; } 
      else { btnPagar.disabled = true; btnPagar.style.opacity = '0.4'; }
  }
}
...
function openPayModal() {
  ...
  if (!termsAgreed) { document.getElementById('termsModal').style.display = 'flex'; return; }
  let container = document.getElementById('selectedChips');
  if(container) {
      container.innerHTML = '';
      Array.from(selectedTickets).sort().forEach(num => {
        let chip = document.createElement('div');
        chip.className = 'selected-chip'; chip.textContent = num; container.appendChild(chip);
      });
  }
...
async function submitOrder(e) {
  ...
  try {
    ...
    const { data: pedidoData, error: pedidoError } = await db.from('pedidos').insert([{
        nombre: nombre, cedula: cedula, whatsapp: whatsapp, ref_comprobante: referencia,
        numeros: boletosArray.map(Number), total: totalPagado, capture_url: captureUrl, estado: 'pendiente'
      }]).select().single();
    if (pedidoError) throw pedidoError;

    const ticketsToInsert = boletosArray.map(num => ({ numero: parseInt(num, 10), estado: 'pendiente', pedido_id: pedidoData.id }));
    ...
    updateSalesBar(); renderGrid();
    try { await notificarTelegram(nombre, boletosStr, totalPagado, referencia); } catch (telErr) {}
    ...
    showToast('❌ Compra detenida. Revisa la conexión.');
    btn.disabled = false; btn.innerHTML = '🚀 Confirmar y Reservar';
  }
}
...
async function notificarTelegram(nombre, boletos, total, ref) {
  ...
  await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: CHAT_ID, text: mensaje }) });
}
...
if (searchInput) {
  searchInput.addEventListener('input', (e) => {
    ...
    if (val.length === 2) {
      if(!availableList.includes(val)) { showToast('Número ocupado o inválido'); return; }
...
async function buscarMisBoletos() {
    ...
    } catch (err) { resultsDiv.innerHTML = '<div style="color:#ef4444;font-weight:700;text-align:center;">❌ Error al consultar.</div>'; } 
...
function showToast(msg, duration = 2000) {
  ...
  t.className = 'toast bg-white border border-pink-200 text-gray-900 shadow-xl'; t.textContent = msg;
  document.body.appendChild(t); setTimeout(() => t.remove(), duration);
}

function startVIPCountdown() {
  let c = 3; let cdNum = document.getElementById('cdNum'); if(cdNum) cdNum.textContent = c;
  let cdRing = document.getElementById('cdRing'); if(cdRing) cdRing.style.setProperty('--pct', '100%');
  if (cdInterval) clearInterval(cdInterval);
  cdInterval = setInterval(() => {
    c--; if(cdNum) cdNum.textContent = c; if(cdRing) cdRing.style.setProperty('--pct', (c/3)*100 + '%');
